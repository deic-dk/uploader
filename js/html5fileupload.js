...
var HTML5FileUpload = {
	...
	phpFile: OC.webroot+'/apps/uploader/ajax/html5fileupload.php', // PHP-Datei, welche die Arbeiten auf dem Server erledigt.
	...
	FolderContainer: document.getElementById('choose_upload_folder'),
	...
	init: function() {
		...
		var loc = OC.webroot+'/apps/uploader/'
		...
		dst.classList.add('target_folder');
		// Set in template main.php
		dst.value = $('#upload_folder').attr('dir');
		dst.placeholder = t('uploader', 'Folder');
		dst.setAttribute('autocomplete', 'off');
		var descr = document.createElement('span');
		descr.innerText = t('uploader', 'Upload destination');
		descr.classList.add('bold');
		var info = document.createElement('span');
		info.innerText = ' ⓘ';
		info.setAttribute('title', 'Optional target folder. A default value can be set in your preferences. If empty, files will be kept in your home root folder.');
		info.classList.add('info');
		this.FolderContainer.appendChild(descr);
		this.FolderContainer.appendChild(info);
		var toggleTargetFolder = document.createElement('span');
		toggleTargetFolder.innerText = ' »';
		toggleTargetFolder.id = 'toggle_target_folder';
		this.FolderContainer.appendChild(toggleTargetFolder);
		this.FolderContainer.appendChild(dst);
		var btn = document.createElement('label');
		btn.className = 'uploader_choose_download_folder btn btn-flat';
		btn.classList.add('target_folder');
		btn.appendChild(document.createTextNode(t('uploader', 'Browse')));
		this.FolderContainer.appendChild(btn);
		...
		this.FolderContainer.appendChild(div);
		$('#uploader_group_folder').appendTo(this.FolderContainer);
		$('#toggle_target_folder').click(function(ev) {
			$('.target_folder').toggle('slow');
		});
		// Uploadbutton
		...
		span = null;
		this.Container.appendChild(document.createElement('br'));
		$('#choose_upload_folder').appendTo(this.Container);
		this.Container.appendChild(document.createElement('br'));
		$('#linkPassText').val("");
	},
	...
	handleDrop: function(evt) {
		...
		document.getElementById('HTML5FileUpload_Upload').classList.remove('hidden');
		...
		if (document.getElementById('HTML5FileUpload_Tbl')) {
			...
			tr.appendChild(th);
			th = document.createElement('th');
			th.innerHTML = t('uploader', 'Sharing link');
			...
			if(!searchParams.has('filetransfer') || searchParams.get('filetransfer')==='false'){
				$('#choose_upload_folder').removeClass('hidden');
			}
		}
		...
		for (i = 0; i < fl.length; i++) {
			...
			if (!fl[i].type || (HTML5FileUpload.allowedFileTypes.indexOf(fl[i].type) == -1 && HTML5FileUpload.allowedFileTypes.length != 0)) {
				console.log("Not accepting file type: " + fl[i].type);
			...
			span.appendChild(document.createTextNode(fl[i].name));
			td.appendChild(span);
	...
	delItem: function(el) {
		...
		var rmi = -1;
		for(var i=0; i<HTML5FileUpload.Files.length; ++i){
			if(el.id=='HTML5FileUpload_tr_'+HTML5FileUpload.Files[i].name){
				rmi = i;
				break;
			}
		}
		if(rmi>-1){
			HTML5FileUpload.Files.splice(rmi, 1);
		}
	},
	// Upload
	...
	uploadBlob: function(Blob, Index, Start) {
		...
		fd.append('group', $('#uploader_group_folder').val() /*$('#download_folder').attr('group')*/ );
	...
	uploadComplete: function(evt) {
		...
		var trId = 'HTML5FileUpload_tr_' + HTML5FileUpload.Files[HTML5FileUpload.Counter].name;
		document.getElementById(trId).className = 'HTML5FileUpload_Complete';
		// Add metadata to row - to be used for sharing
		var json = $.parseJSON(this.responseText);
		console.log('#'+trId+' --> '+json['fileid']);
		document.getElementById(trId).setAttribute('filename', HTML5FileUpload.Files[HTML5FileUpload.Counter].name);
		document.getElementById(trId).setAttribute('fileid', json['fileid']);
		document.getElementById(trId).setAttribute('mimetype', json['mimetype']);
		document.getElementById(trId).setAttribute('path',  document.getElementById("HTML5FileUpload_destdir").value);
		document.getElementById(trId).setAttribute('group', $('#uploader_group_folder').val());
		...
		if (HTML5FileUpload.Counter < HTML5FileUpload.Files.length) {
			...
			document.getElementById('HTML5FileUpload_Reset').style.display = 'inline';
			if (window.Notification) {
				var notification = new Notification(HTML5FileUpload.lang['notification_title'], {
					dir: "auto",
					lang: "",
					body: HTML5FileUpload.lang['notification_body_done']
				});
			}
			//alert(HTML5FileUpload.lang['done']);
			HTML5FileUpload.UploadInProgress = false;
			$('.HTML5FileUploadShare_Button').prop("disabled", false);
			$('.uploader_share').removeClass('hidden');
			var searchParams = new URLSearchParams(window.location.search);
			if(searchParams.has('filetransfer') && searchParams.get('filetransfer')!=='false'){
				$('.uploader_share_meta').show('slow');
